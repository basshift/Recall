...
pub fn build_tri_grid(state: &Rc<RefCell<AppState>>) -> gtk::Grid {
    ...
    let update_styles = {
        ...
        move |grid: &gtk::Grid| {
            ...
            if width > 0 && height > 0 {
                let cell_width = (width - (grid_cols - 1) * super::board::TILE_GAP) / grid_cols;
                let cell_height = (height - (grid_rows - 1) * super::board::TILE_GAP) / grid_rows;
